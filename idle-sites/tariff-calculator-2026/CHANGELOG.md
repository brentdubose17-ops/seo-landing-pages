...
- **Known durability caveat:** `how-to-calculate-import-duties-from-china.html` is **regenerated** by `all-sites-content.py` (its topic is in that site's list), so this hand-edit is not durable against the next run for that topic — the same trap recorded on 2026-09-15. The writer-side fix belongs with the generator, not the page; flagged rather than silently assumed.
