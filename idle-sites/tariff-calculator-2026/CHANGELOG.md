...
- **Every fact re-probed first-hand before publishing, not inherited from the card.** White House statement HTTP **200**, 262,318 B, `article:published_time 2026-09-18T21:43:23+00:00`, body *"On Friday, September 18, 2026, the President signed into law: H.R. 5334, the "Lindsey O. Graham Sanctioning Russia and Iran Act of 2026," …"*; Senate **Record Vote 224** XML HTTP 200 — 100 members, **86 Yea / 11 Nay / 3 Not Voting**, *"On Passage of the Bill"*, August 7, 2026; Clerk **roll 308** XML HTTP 200, 82,993 B — `H R 5334`, *"On Motion to Concur in the Senate Amendments"*, `16-Sep-2026`, `Passed`, `totals-by-vote` **262 / 159 / 0 / 12**, by party **R 203/7/8 · D 58/152/4 · I 1/0/0**; Clerk **roll 300** XML HTTP 200, 82,816 B — **H RES 1530**, *"On Agreeing to the Resolution"*, **Passed 214–211–0–8**; govinfo PLAW collection **count 0**; Federal Register **name-restricted (phrase-quoted) query count 0**. One measurement worth keeping: the *unquoted* `conditions[term]` form of that FR query returns **211 unrelated documents**, so only the phrase form tests the Act's name — the card's "0" is the phrase form, and the page carries it that way.
- **Ceilings only — no rate asserted, no public-law number asserted.** The section says the §112 duty is **"up to 500 percent ad valorem"**, imposed **"within 30 days of enactment"**; §113 **"up to 100 percent"** on **up to five described countries**; **"no country is named in the statute"**; §112 sets **"no minimum"**; §115 waiver / §117 termination / 5-year sunset (§203); and the tracker lag in one dated bullet (no public-law number published as of 2026-09-19). No sentence says the 500 percent duty "is in effect" or "took effect on signature" (asserted absent in the shipped bytes).
- **One correction against the card, taken from the primary record:** roll 300's own `action-date` is **15-Sep-2026** — the card grouped the rule with the House's 16-Sep concurrence. The page prints **Sept 15, 2026** for H. Res. 1530 and **Sept 16, 2026** for the concurrence; the hub reads the same way.
- **Bookkeeping moved in lockstep:** `<meta name="dateModified">` **2026-09-10 → 2026-09-19**, the visible byline **`Updated September 14, 2026` → `Updated September 19, 2026`**, and both description metas re-led to **"New (Sept 19)"** (the Sept 3 drone lead they replaced still lives in the `Earlier advisories & explainers` feed below). The three existing advisory sections, the headings and the feed rows are otherwise untouched.
- **Gates, before and after:** `site_consistency.py news/index.html --expect-date 2026-09-19` → **1 checked / 0 error(s) / 0 warning(s)**, identical under `--strict`; publisher `--dry-run` on the same HEAD → **47 checked / 0 error(s) / 1 warning(s) / 1 pre-existing baselined**, i.e. the same reading as the pre-edit tree — **0 new errors** (the single warning is the long-standing cosmetic FAQ quote-character pair on `fmcsa-motus-phishing-sites-biennial-update-pause.html`). Tag balance on the shipped bytes: div/ul/ol/li/p/h2/h3/span all balanced, **4** advisory blocks, **4** sources blocks, **1** `<h1>`.
- **Commit + deploy:** commit **`bf4fd34`** (`git commit --only`, **1 file / 48 insertions / 4 deletions** — `git status` was checked before and after and held only that path, so the six peer `*.bak2` residue files stayed untracked and nothing else rode along). Deploy **`b542fa16-ca83-48a6-9ec4-f23ccc8a3b93`** (project `tariff-calculator-2026`, branch `main`, `source: head`, 60 staged files, gate **0 errors / 1 warning / 1 pre-existing baselined**, **59 live rows compared / 0 mismatches / 0 redirect-served / 1 control-file skip**). `news/index.html` **37,819 B `e18e81fc1c79b3eb…` → 46,530 B `e7348585e0a89688…`**.
- **Verified live, independently of the publisher:** `/news/` **HTTP 200**; the served bytes canonicalise **byte-identically to the committed revision** (raw live 46,897 B under a browser UA — +367 B of CF edge beacon — → `strip_cf_edge_injection()` gives **46,529 B / sha16 `5ecdb98677c03c54` on both sides**, and `git show HEAD:` is the same 46,530 B file). The no-slash form still **308 → `/news/`** and the canonical is still `https://tariffcalculator2026.com/news/`, so `t_05241b49` is not regressed. On the **served** bytes: `262–159` ×3 · `roll 308` ×3 · `86–11` ×3 · `September 18, 2026` ×5 · `within 30 days of enactment` ×5 · `5334` ×11 · `§112` ×9 · `§113` ×7 · exactly **one** `Client Advisory: H.R. 5334` section (the first H2 on the page) · **0** hits for `pending in Congress` / `has not voted` / `awaiting a House vote` · no `Public Law No` / `PL 119` pattern · visible `Updated September 19, 2026` == meta `dateModified 2026-09-19`. The link target `https://tariffcalculator2026.com/presidential-tariff-authority-2026/` is **200** and unchanged (36,353 B raw → canonical **35,985 B / `03fd1e4626664cdf`**, the same canonical hash the publisher's manifest carries).
- **Durability, measured not assumed:** `grep -n "news" ~/.hermes/scripts/all-sites-content.py` → **0 matches**, and the `tariff-calculator-2026` entry's 14 `topics` (lines 636–651) are the flat explainer pages only — so `news/index.html` is **not** regenerated by the writer's rotation and a hand-landed section here cannot be lost to a regeneration turn. `sitemap.xml` was **not** touched (`t_43315060` owns a sitemap `<lastmod>` pass on this site; its `/news/` `<lastmod>` still reads 2026-09-14). `llms.txt` needed no change — its `/news/` entry already exists and the publisher regenerates the index from the staged tree on every publish.
- **Not done here, on purpose:** `/presidential-tariff-authority-2026/` (already corrected), the 11 sibling pages (already corrected), `/tariff-refund-status-2026/` (`t_a4e412d3`), `sitemap.xml`, and the six peer `*.bak2` residue files in the site dir.

## 2026-09-19 — /tariff-refund-status-2026/ rewritten in place for CAPE Phase 3 and the October 6, 2026 date: the "no deadline exists" framing retired, the page split into the two reader pathways, and the stale H.R. 5334 fragment cleared (kanban t_a4e412d3)

- **What shipped:** the refund hub's in-place rewrite. The retired framing is gone — the `⚠️ CLOSING, NOT CLOSED — AND NO DEADLINE EXISTS` H2 **and all three wordings** of the no-deadline claim measured on the page (`no source anywhere publishes a portal deadline`, `This page publishes no portal deadline, because no source publishes one.`, `No portal deadline has been published as of September 11, 2026`, plus the meta-description and FAQ instances). In their place: an answer block plus a **status banner** stating that a Phase 3 date now exists *for one population*, and a **two-pathway split** — **Pathway A** = reliquidation plaintiff with a valid importer of record number submitted to CBP by **July 30, 2026**, who can file CAPE Declarations from **October 6, 2026**; **Pathway B** = everyone else, split into **B1** (IOR provided after July 30 → CBP's further instructions, no date) and **B2** (not a reliquidation plaintiff at all → no Phase 3 route; the pending class-certification motion is the mechanism). H1, title tag and meta description are the targeting brief's frozen values: H1 98c, title 58/60 *IEEPA Tariff Refund Deadline: CAPE Phase 3 Opens October 6*, meta 153/155.
- **Sourcing discipline — October 6 is a court filing, never an announcement.** The date is attributed exactly as the record has it: *CBP stated in a September 15, 2026 court filing* — the Declaration of Brandon Lord, Executive Director of Trade Programs, Ct. No. 26-01088 (Doc 50), in *Freestyle World, Inc. v. U.S. Customs and Border Protection*. No sentence says CBP announced or published it, and none implies CSMS carried it. A dedicated section explains the agency-page gap for the reader who checks: CBP.gov's IEEPA Duty Refunds page carries `Last Modified: Sep 02, 2026`, `Phase 3` **0 times**, and still lists `Entries for which liquidation is final.` among future deployments (re-measured in this run). The **August 25** delay (Doc 38 ¶8) is printed as **history** in a two-row dated-state table, with the September 15 date as current — the "delay" is not presented as the live state.
- **Numbers, with basis labels and the as-of stamp.** A §5 table added with CBP's own figures as of **3 p.m. ET, September 11, 2026**: **286,044** declarations submitted / **201,293** past file validations / declarations that passed file validations **cover 27.2 million entries** / **6.1 million** entries failed the entry-level validations / **19.94 million** accepted entries since liquidated or reliquidated without IEEPA duties / about **$134.7 billion** in potential and certified refunds accepted for processing / about **$122 billion** certified and completed (duties plus interest) / **20,184 refunds, approximately $1.3 billion, not transmitted to Treasury** because *Automated Clearing House account information has not been provided by the importer of record or its authorized CBP Form 4811 designee* — **not** "bad payment information", and **not** "processed through CAPE" (CBP's verb is *cover*, and only for file-validated declarations). The `$122B` (certified, Doc 50 ¶5) and the page's `$115.078B` (Treasury MTS paid) are kept in **separate sections on separate bases** and are never netted; the older secondary pair the page carried (**22,170 refunds / ~$1.7B**, Aug 2026 basis) is replaced by the September 11 primary pair.
- **Legal layer added (§6):** the **pending nationwide class-certification motion under Rule 23(b)(2)** — *V.O.S. Selections, Inc., et al. v. Donald J. Trump, et al.*, No. 25-00066-RKE (Ct. Intl. Trade), Doc 99, filed **2026-06-04**, oral argument held **2026-08-06** (parallel class motion, *Freestyle World*, Doc 18, filed 2026-06-23) — described as what it is: a **motion that seeks** to let importers recover refunds on finally liquidated entries without each filing an individual suit, **not granted**, and with a class defined by CAPE *ineligibility* (broader than "finally liquidated entries"). Plus the **protest** boundary, sourced to Kelly M. Reid (Ward and Smith), *After Learning Resources: Liquidation, Protests, and IEEPA Tariff Refunds*, Sept 18, 2026: the 180-day protest period does not answer the Phase 3 problem, and such a protest is not established as required-to-preserve or sufficient-to-recover for the pure IEEPA claim.
- **The stale H.R. 5334 fragment from the `t_7ed04907` sweep is cleared (this card inherited it via comment).** The timeline's authority note read `a fifth mechanism is now pending in Congress`; it now reads **`as of September 19, 2026 a fifth mechanism is law: H.R. 5334, the Lindsey O. Graham Sanctioning Russia and Iran Act of 2026, signed into law on September 18, 2026, with ceilings of up to 500 percent on Russia and up to 100 percent on the secondary measure. No public-law number is published yet, and this page asserts none.`** — matching the hub's `Status: SIGNED INTO LAW — as of September 19, 2026 (ET)`, and deliberately opening with the `as of <Month D, YYYY>` form the writer's own carry predicate scores `True` (so the clause survives a regeneration turn even though this page's topic is not in the rotation). The sweep leg's earlier "do **not** write 'signed into law'" instruction was re-measured first-hand and is **superseded**; no public-law number is asserted anywhere.
- **One acceptance string in the card was NOT satisfied, on purpose, and the gate is the thing that is wrong — not the page.** The card asked for the word **`omnibus`** (and the sibling publish card greps for it). That term appears in **zero** of the 47 evidence artifacts **and** zero of the 25 competitor pages the targeting leg fetched; the fact sheet marks it **CONTRADICTED** and the card's own body says to omit anything the fact sheet marked unverified. The instrument is a **class-certification motion**, and that is what the page says — `class-certification` **8** occurrences in the rendered text. All three upstream legs (`content-researcher`, `research-seo`, and this profile's own sweep comment) independently asked for the acceptance gate to grep `class-certification` instead. **Requested gate amendment: `omnibus` → `class-certification`.**
- **Gates (all on the exact bytes that shipped):** research-pack `page_gate.py --file` → **all 7 required strings present with counts, all 7 retired wordings 0, FAQPage 12 questions, both JSON-LD blocks parse, Article.headline == the frozen H1**; its description leg reports a **false FAIL** because its extractor `content=["\'](.*?)["\']` terminates at the first `'`/`"` and the frozen description contains the apostrophe in "CBP's next instructions." — the corrected comparison (`html.unescape` both sides, attribute delimited by its own quote character) reads **title 58/58 `match=True`, description 153/153 `match=True`**, so this is a known instrument bug class, not page drift. `site_consistency.py` on the site dir: **47 checked, 0 errors / 1 warning / 1 pre-existing baselined — identical to the pre-edit reading**, and the only warning is the long-standing cosmetic quote-character pair on `fmcsa-motus-phishing-sites-biennial-update-pause.html`. `faq-parity-sweep.py` on the file **and on the served bytes** → **pairs {'MATCH': 12}, 0 class-(a) drift, 0 convention-only, 0 order-only**. Real Chromium at **320 / 390 / 1280 px** → `documentElement.scrollWidth == innerWidth` at every width, **0 offending elements**, 1 `<h1>` / 12 `<h2>` / 12 `.faq-item` / 5 tables. Word count **4,915 visible body words** (brief ceiling 4,500; the overage is 415 words, ~9%, and it is deliberate — the alternative was cutting either the 12-pair FAQ the brief froze at 992 words or the required timeline rows).
- **Byte deltas:** `tariff-refund-status-2026/index.html` **38,505 B → 56,991 B** (+18,486; pre-edit copy kept beside it as `index.html.bak-t_a4e412d3-20260919T0250Z`, pruned from the artifact by policy), and `llms.txt` **5,698 → 5,722 B** (+24) — the index entry's label was the last live surface still carrying the retired claim (`IEEPA Tariff Refund Status 2026: Still Open, No Portal Deadline` → `IEEPA Tariff Refund Deadline: CAPE Phase 3 Opens October 6`). `sitemap.xml` was **not** touched: its `<lastmod>` for this URL reads 2026-09-15 and sitemap reconciliation is owned by `t_290898c5` per the `t_2543361d` ruling.
- **Commit + deploy:** commit **`371f663`** (path-scoped `git commit --only`, 2 files / 153 insertions / 66 deletions — `git status` was checked before and after and held **only these two paths**, so no sibling's in-flight edit rode along and the six peer `*.bak2` files stayed untracked). Pre-deploy `--dry-run` on the same HEAD: gate **47 checked / 0 error(s) / 1 warning(s) / 1 pre-existing baselined**. Deploy **`a348b55d-dddb-42e8-a576-d2cdc7d06da9`** (project `tariff-calculator-2026`, branch `main`, `source: head`, commit `371f663`, 60 staged files, gate `{errors: 0, warnings: 1, baselined: 1}`, **59 live rows compared / 0 mismatches / 0 redirect-served**; `declared_overlay` / `in_flight_not_published` / `live_but_uncommitted` **all empty**).
- **Verified live, independently of the publisher:** the served bytes canonicalise **byte-identically to the committed revision** — staged 56,991 B sha256 `4a80b6c6…` == `git show HEAD:` blob, raw live 57,228 B (+237 B CF edge injection) → `strip_cf_edge_injection()` gives **56,990 B / sha16 `f92399e0bbade4d3` on both sides**. On the **served** page: `Phase 3` **39**, `October 6` **23**, `July 30` **16**, `27.2 million` **3**, `$134.7 billion` **2**, `20,184` **4**, `class-certification` **8**, `protests` **3**, `importer of record` **22**; **0** hits for every retired wording; visible `Updated September 19, 2026` **==** JSON-LD `dateModified 2026-09-19` (`datePublished` 2026-09-11 unchanged); 12 visible FAQ items == 12 FAQPage questions with **0 drift** on the live bytes; `/tariff-refund-status-2026/` **200**, extension-less **308** to the slash URL, `/index.html` **308** to the slash URL. `llms.txt` live (5,722 B) carries the new label. IndexNow for the updated URL: `api.indexnow.org` **200**, `bing.com/indexnow` **200** (key file verified live 200 with body == key in the same run).
- **Not done here, on purpose:** `sitemap.xml` (owned by `t_290898c5`); the peer `*.bak2` files left in the site dir (a sibling's, excluded by policy and dropped from the artifact); and GC/IndexNow submission to Google Search Console, which remains impossible from this host (`invalid_grant` — a credentials gap, recorded as such).

## 2026-09-19 — H.R. 5334 stale status corrected on the homepage + the 11 sibling pages: `pending in Congress` / `awaiting a House vote` → signed into law (kanban t_da62362c)

- **Premise re-derived first-hand, not inherited from the card — and the card was wrong.** `t_da62362c` was written against "enrolled / presented to the President, **NOT YET LAW**" and its binding constraint #1 forbade the words "signed into law". Re-probed at 02:3x ET and that premise is **false**: `whitehouse.gov/briefings-statements/2026/09/congressional-bill-h-r-5334-signed-into-law/` is **HTTP 200** with `article:published_time 2026-09-18T21:43:23+00:00` (17:43 ET) and the operative sentence **"On Friday, September 18, 2026, the President signed into law: H.R. 5334, the "Lindsey O. Graham Sanctioning Russia and Iran Act of 2026," which authorizes and expands statutory sanctions, tariffs, and prohibitions on Russia and extends existing sanctions on Iran."** Corroborated by `clerk.house.gov/evs/2026/roll308.xml` → `legis-num H R 5334`, `vote-question "On Motion to Concur in the Senate Amendments"`, `action-date 16-Sep-2026`, `action-time 6:28 PM`, `vote-result Passed`, `totals-by-vote 262 / 159 / 0 / 12`. **The card's constraint was therefore not followed**: writing "not yet law" on 11 live pages would have shipped a falsehood. The hub's own wording (`t_26b5a4dd`, same day) is the anchor the card's constraint #3 required consistency with — `Status: SIGNED INTO LAW — as of September 19, 2026 (ET)` — and these 11 pages now match it. The federal trackers still lag (congress.gov `To President`, govinfo PLAW 0, no public-law number), which is why the pages say **signed into law** on the White House record and nothing about a public-law number.
- **The edit, bounded to one status fragment + the containing block's date stamp per page, 11 files:** the homepage banner `NOT YET LAW — as of September 15, 2026` → `SIGNED INTO LAW — as of September 19, 2026` with `The House has not voted on it as of September 15, 2026, and it has no legal effect today.` → `The House concurred in the Senate amendments 262–159 on September 16, 2026 and the President signed it on September 18, 2026.`, and its hub link parenthetical `(H.R. 5334: not yet law)` → `(H.R. 5334: signed into law)` (+ the tail clause `what is still pending in Congress` → `what the new statute sets next`, since the bill is no longer pending). On the 10 article/directory pages the clause `A fifth mechanism is pending in Congress: …` / `is pending in Congress and is not law as of September 15, 2026` became `A fifth mechanism is now law: … was signed into law on September 18, 2026` (each page keeping **its own** mechanics verbatim), and each `Authority update / note / comparison (September 15, 2026)` stamp became **(September 19, 2026)**. Both `how-to-calculate-import-duties-from-china` copies — the visible FAQ answer **and** its FAQPage JSON-LD twin — were replaced by the **same** string in one pass. Two adjacent-word status claims were corrected with them because they asserted the same falsehood: section-232's `the proposed Russia sanctions authority` → `the now-enacted Russia sanctions authority`, and section-338's `would delegate` → `delegates`.
- **Ceilings and mechanics untouched (constraint #2), verified by diff:** every `up to 500 percent` / `up to 100 percent` / "up to five other countries" ceiling, the `§112`/`§113` mechanics, the `§115` waiver / `§117` termination / 5-year sunset text and the no-prediction rule are **byte-identical** to their pre-edit values; the surrounding Authority blocks, tables and page prose are otherwise unchanged. No page asserts a signature date other than **September 18, 2026**, and **no** page says "signed into law" as a forecast — the White House statement is the record.
- **Date surfaces moved in lockstep (the fleet's "new copy + stale dateModified" incident shape):** `dateModified 2026-09-15 → 2026-09-19` on the 10 article/directory pages, and the visible `Updated September 15, 2026` byline → `2026-09-19` on the **exactly 3** pages that carry one (`section-338-…`, `section-232-metals-tariffs`, `tariff-dividend-5000-explained/`). The homepage's JSON-LD was already `2026-09-19`. The flat pages' meta/og/JSON-LD descriptions say `Updated September 2026` (no day) and were left alone.
- **Byte deltas (worktree, pre → post):** `index.html` 260,066 → 260,104 (+38) · `china-tariff-rates-2026-explained.html` 32,704 → 32,730 (+26) · `tariffs-on-electronics-imports-from-asia-2026.html` 34,933 → 34,959 (+26) · `how-to-calculate-import-duties-from-china.html` 37,590 → 37,738 (+148) · `de-minimis-cit-ruling-2026-importers.html` 18,971 → 19,023 (+52) · `tariff-exemption-list-2026-what-products-are-exempt.html` 38,825 → 38,804 (−21) · `tariff-impact-on-us-manufacturing-supply-chains-2026.html` 24,476 → 24,455 (−21) · `us-tariff-rates-2026-by-country.html` 38,404 → 38,383 (−21) · `section-338-canada-import-ban-usmca-legal-challenge.html` 39,082 → 39,079 (−3) · `section-232-metals-tariffs.html` 16,387 → 16,476 (+89) · `tariff-dividend-5000-explained/index.html` 56,482 → 56,475 (−7). Pre-edit copies kept beside each file as `*.bak-t_da62362c-20260919T0245Z` (pruned from the artifact by policy).
- **Gates (all on the edited bytes):** `site_consistency.py <file> --expect-date 2026-09-19` → **11/11 files: 1 checked, 0 error(s), 0 warning(s)**; `faq-parity-sweep.py` run **per page** (it takes one positional target — extra paths are silently ignored) → **0 class-(a) drift, 0 convention-only, 0 order-only** on all 11, FAQ pairs MATCH 11/6/7/8/4/8/0/6/7/2/6. Stale-needle census after the edit: `has not voted` / `awaiting a House vote` / `pending in Congress` / `not law as of` / `NOT YET LAW` / `has no legal effect` → **0 occurrences on all 11 files**. The single surviving `not yet law` on the homepage is the **unrelated** Canada Productivity Mega Deduction `BLOG_ARTICLES` entry (line ~1795) and was deliberately not touched.
- **Deploy:** `1027be03-3be0-4af6-9e57-cb222d18f10a` (project `tariff-calculator-2026`, branch `main`, commit **`b089b82`**, publisher `source: head`, 60 staged files, gate **0 errors / 1 pre-existing baselined / 1 pre-existing cosmetic warning**, **59 live rows compared / 0 mismatches / 0 redirect-served**, `declared_overlay` / `dropped_dirty` / `in_flight_not_published` / `live_but_uncommitted` **all empty** — nothing else rode along). Independently verified after the deploy: a plain `curl` (no browser UA, therefore no CF edge injection) of the canonical URL for **all 11 pages is HTTP 200 and sha256-identical to `git show HEAD:<path>`** (homepage 260,104 B `ad893ac16c25950d`; how-to 37,738 B `5b871321b797f0d4`; china 32,730 B `ead008da72ba84fb`; electronics 34,959 B `b695bd30c0e40bca`; de-minimis 19,023 B `a4a15acd5d0288c8`; exemption 38,804 B `acf90e6ca6d898ff`; manufacturing 24,455 B `a697df8af3e2ab66`; us-rates 38,383 B `b576aabf29c224a3`; section-338 39,079 B `40d443f9586db18b`; section-232 16,476 B `9b8e3f54ffa9f6e2`; dividend 56,475 B `332a834484ed1e1c`). The served bytes carry `signed into law` and none of the stale needles; the live homepage hub anchor reads `presidential tariff authority — every source, compared (H.R. 5334: signed into law)`; the live `how-to-calculate` FAQ answer is **byte-identical** between the visible copy and its FAQPage `acceptedAnswer.text` (621 == 621 B); an unknown path returns **404 / 1,574 B** (the site really does 404); and a real Chromium render at **320 / 390 / 1280 px** on 4 sampled live pages gives `documentElement.scrollWidth == innerWidth` with **0 offending elements**. Committed **before** deploying, path-scoped (`git commit --only`), because the publisher stages the whole tree from `git HEAD` and the `~/seo-pages` index is shared with concurrent workers. `index.html` is a known **hotspot** (`t_f8163375`, status `ready`, edits the different `#ior-survival-checklist` section of the same 260 KB file): `git status` was checked before and after and held **only these 11 paths**, so no half-written homepage and no sibling's in-flight edit rode along.
- **Not done here, on purpose:** `/presidential-tariff-authority-2026/` (owned and already fixed by `t_26b5a4dd`), `/tariff-refund-status-2026/` (`t_a4e412d3`, CAPE Phase 3; its fragment is `a fifth mechanism is now pending in Congress`, L122, and was **left untouched**), and historical CHANGELOG entries. `llms.txt` / `sitemap.xml` carry no status text and needed no change.
- **PASS 2 — durability against the writer's rotation, measured not assumed.** `how-to-calculate-import-duties-from-china` (and 5 further pages in scope: `china-tariff-rates-2026-explained`, `tariffs-on-electronics-imports-from-asia-2026`, `tariff-exemption-list-2026-what-products-are-exempt`, `tariff-impact-on-us-manufacturing-supply-chains-2026`, `us-tariff-rates-2026-by-country`) carry a topic that **is** in `all-sites-content.py`'s `tariff-calculator-2026` `topics` list (lines 636–651), so they are **regenerated** on rotation and a hand-landed prose sentence survives only if it satisfies the writer's own carry predicate. Controlled against the writer's real functions (loaded via `importlib` from `~/.hermes/scripts/all-sites-content.py`, not re-implemented): **the pass-1 sentences scored `is_standing_qualification() = False`** — a date but no "as of <Month D, YYYY>" and no status-change phrase — so `standing_qualifications()` inventoried **0** of them and the next rotation turn for that topic would have dropped the correction silently (exactly the 2026-09-15 loss this mechanism was built for). Each of the 6 was therefore re-worded to open **`As of September 19, 2026 a fifth mechanism is law: …`**, which scores **True**; re-running the writer's own `standing_qualifications()` over the on-disk pages now inventories the H.R. 5334 status sentence on **6/6** (inventoried counts 2→3, 2→3, 1→2, 2→3, 2→3, 5→6), so the fact is now carried **verbatim** across a regeneration instead of being lost. The controls behave: the pre-existing IEEPA sentence still trips it, the rhetorical "two of its layers no longer rest on that authority" still does not. This also discharges the card's acceptance that the hub, the homepage and the 11 pages carry **one as-of date**: every in-scope page now states **as of September 19, 2026** in prose (previously `how-to-calculate` carried only an ISO `dateModified`). Pass-2 deltas: china +21 B, electronics +21, how-to +62, exemption +21, impact +21, us-rates +21. Shipped as commit **`266b44b`** (path-scoped, 7 files / 8 insertions / 8 deletions) and deploy **`8231d20a-a361-4264-ab86-3637139ed067`** (60 staged files, gate 0 errors / 1 pre-existing baselined, **59 live rows compared / 0 mismatches**, all four dirty/overlay lists empty); re-verified live afterwards — all 11 pages HTTP 200 and sha256-identical to `git show HEAD:<path>` (china 32,751 B `e4a44caff1737a51`; electronics 34,980 B `c917f0f4be09c6d6`; how-to 37,800 B `1fcb3de0c19f3640`; exemption 38,825 B `21caaa66c6bde761`; manufacturing 24,476 B `bd064aab14fae4ad`; us-rates 38,404 B `caaf3b1345325dcd`), and the live `how-to-calculate` visible-FAQ/JSON-LD pair is still byte-identical (652 == 652 B).

## 2026-09-19 — /presidential-tariff-authority-2026/ corrected in place: H.R. 5334 is LAW (signed 2026-09-18), House roll 308 in, three false Clerk-ceiling claims out (kanban t_26b5a4dd)

- **Premise flip, from the primary record.** The correction card was written against "enrolled / presented to the President, NOT YET LAW" (measured 2026-09-18 19:1x ET). That was already false: **`whitehouse.gov/briefings-statements/2026/09/congressional-bill-h-r-5334-signed-into-law/`** (HTTP 200, 262,318 B on disk, sha256-16 `0d54202109415c1f`, `article:published_time 2026-09-18T21:43:23+00:00` = 17:43 ET) reads **"On Friday, September 18, 2026, the President signed into law: H.R. 5334, the "Lindsey O. Graham Sanctioning Russia and Iran Act of 2026," …"** — re-derived first-hand at 02:22 ET today, corroborated same-day by AP/Reuters/CNBC/Al Jazeera. The federal trackers have **not** caught up (congress.gov still "To President"/"09/17/2026 Presented to President", govinfo PLAW search count 0, no public-law number published) — so the page says so, dated, instead of inferring "not law" from surfaces that lag.
- **What changed (one pass, 16 dated copies — 33,074 B `8a0adfe4e91ccc62` → 35,986 B `4d20ed715112 92b0`):** the status heading is **`Status: SIGNED INTO LAW — as of September 19, 2026 (ET)`**; the three Clerk-counter claims (**"nothing above 307"**, **"the Clerk's highest 2026 roll call is 307"**, **"highest 2026 roll call: 307"** on lines 77/160/181) are **deleted, not re-stamped** — they were false again at 307 → 314, the third instance of that churn class — and replaced by the bill-specific record: **Clerk roll call 308, 262–159, 16-Sep-2026 18:28 ETZ, "On Motion to Concur in the Senate Amendments"**, plus the dated sequence Senate 86–11 (Aug 7, Record Vote 224) → House concurred (Sept 16) → signed (Sept 18). Every "the House has not voted" / "nothing has been signed" sentence is gone, the "What would have to change before it takes effect" list became "From passage to effect", and the no-prediction rule survives in a truthful form (the page forecasts no list, rate or waiver). `dateModified` **and** the visible byline moved to **2026-09-19** (`datePublished` stays 2026-09-15), FAQ visible answers and FAQPage JSON-LD moved in lockstep (**9/9 verbatim-equal, 6 answers re-answered**), and the Federal Register line was re-run and re-dated (**name-restricted count still 0 on 2026-09-19**), with the unsound inference removed: the FR publishes rules and notices, not enacted statutes, so a zero count was never evidence of "not signed". Sources gained the White House statement, **`clerk.house.gov/evs/2026/roll308.xml`** and the enrolled text.
- **Registry re-point (mandatory, same pass):** `~/.hermes/seo-data/dated-page-freshness.json` → `dated_section_marker` **`"Status: NOT YET LAW"` → `"Status: SIGNED INTO LAW"`**. The old literal is load-bearing for both the daily gate (`d7a34caf0a7d`) and the watcher's `page_asof()`; a marker that has become false would have turned the entry into NO-SECTION, so the heading and the field moved together (backup `dated-page-freshness.json.bak-t_26b5a4dd-20260919T022737Z`); an `observed_moves` entry and the `routed_cards` row were added. The playbook gained a new **§0.0 STATE 2026-09-19** section above the superseded one.
- **Every figure re-derived independently, not inherited from the card:** `roll308.xml` (82,993 B on disk, sha16 `6d4db5c592473429`) → `legis-num H R 5334`, `Passed`, `16-Sep-2026`, `time-etz="18:28"`, totals-by-vote **262/159/0/12**, by-party **R 203/7/0/8 · D 58/152/0/4 · I 1/0/0/0**; Clerk index max **314** (rolls 298–314); Senate roll 224 (86–11); FR name-restricted **0**, bare endpoint **10000**; govinfo PLAW **0**; enrolled text `BILLS-119hr5334enr` **200**, `dc:title "HR 5334 ENR: Lindsey O. Graham Sanctioning Russia and Iran Act of 2026"`. The mechanism caption's claim that the Senate-passed text carries the same §§112–113 language was **measured**: from `112. Increase in duties…` to the end of both texts, ENR and EAS are **identical after whitespace normalisation** (first divergence is Division B vs the title-amendment tail), and `to a rate of up to 500 percent ad valorem` / `…up to 100 percent ad valorem` are present in **both**.
- **`H. Rept. 119-825` verified rather than inherited-flagged:** the evidence leg could not resolve it (congress.gov's *bill* header prints `H. Rept. 119-600`, the Ways and Means report). Re-checked today: `rules.house.gov/bill/119/hr-5334` prints **H. Rept. 119-825** and `govinfo.gov/content/pkg/CRPT-119hrpt825/pdf/CRPT-119hrpt825.pdf` is a real 5-page package — the page's citation stands, unedited.
- **Gates, before and after (all on the exact bytes that shipped):** `site_consistency.py <file> --expect-date 2026-09-19` → **1 checked / 0 error(s) / 0 warning(s)**; `faq-parity-sweep.py` → **9 MATCH / 0 class-(a) drift / 1 clean**; real-Chromium render at **320 / 390 / 1280 px** → `scrollWidth == innerWidth`, **0 offenders**, 0 console errors, 9 faq-items / 2 tables; `dated-page-freshness-check.py --id …` → **OK, as-of 2026-09-19 (via as-of line)**; `hr5334-tariff-status-watch.py` → `page_marker=ok`, `page_asof=2026-09-19`, **`page_ceiling_claims=none`** (the deleted ceilings, by design), rc 0.
- **Deploy:** `4e059d98-d999-4793-9e94-66ae6762cf4c` (project `tariff-calculator-2026`, branch `main`, commit **`8e347be`** — the page **and** this CHANGELOG, path-scoped `git commit --only`, 2 files / 42 insertions / 29 deletions; publisher log `source: head`, 60 files, `gate_errors 0 / gate_baselined 1`, **59 live rows compared, 0 mismatches, 0 redirect-served**). Committed **before** deploying, deliberately: the publisher stages the whole tree from `git HEAD`, and `declared_overlay` / `dropped_dirty` / `in_flight_not_published` / `live_but_uncommitted` all came back **empty** — nothing else rode along. A pre-deploy `--dry-run` on the same HEAD read `47 checked / 0 error(s) / 1 warning(s) / 1 pre-existing baselined`; the one warning is the long-standing cosmetic quote-character FAQ pair on `fmcsa-motus-phishing-sites-biennial-update-pause.html`, untouched here. (The site manifest was copied to the card's evidence dir before that read-only run, because a `--dry-run` overwrites it.)
- **Verified live, independently of the publisher:** a plain `curl` (no browser UA) of the slash URL is **HTTP 200** and **byte-for-byte identical** to the committed file — **35,986 B, sha256 `4d20ed71511292b0…` on both sides** — and a browser-UA fetch adds exactly **+367 B** of CF edge beacon which canonicalises back to the same bytes (`03fd1e4626664cdf` both sides). The served bytes carry the new heading, roll 308 / 262–159, Record Vote 224 / 86–11, the 2026-09-19 last-verified stamp and the new sources, and are **free of** `NOT YET LAW`, `not law as of`, `the House has not voted`, `nothing is signed` and all three `307` ceiling claims. Live FAQ: **9 visible == 9 FAQPage**, 0 drift. `/index.html` 308s to the slash URL (200). IndexNow for the updated URL: `api.indexnow.org` **200**, `bing.com/indexnow` **200** (key file live 200 in the same run).
- **Render on the shipped bytes:** real Chromium at **320 / 390 / 1280 px** → `documentElement.scrollWidth == innerWidth`, **0 offending elements**, **0 console errors**, 9 `.faq-item` blocks / 9 `<h2>` / 2 tables.
- **Not done here, on purpose:** the 11 unowned sibling pages carrying the same stale pre-Sept-16 status are **`t_da62362c`**; the `/tariff-refund-status-2026/` fragment is `t_a4e412d3`; and the watcher's `govinfo-enrolled-bill-exists` / `house-roll-calls-on-the-bill` legs are now permanently true, so it reports `status=CHANGED` on every run — the weekly gate is `enabled=false` and the monthly re-verify (`4b17fd60fa89`, 2026-10-01 09:15) is the only armed catch, so those legs should be re-scoped before the weekly gate is re-armed.

## 2026-09-19 — Post-deadline flip: the Sept-18 CBP Form 5106 / IOR-void hub row is now archived (kanban t_56b4971e)

- **What shipped:** the date-gated status row `#ior-5106-deadline` — the FIRST `.notice-banner` on the hub, added by `t_b5da7757` and live as `data-state="active"` since 2026-09-15 — is flipped to its archived state now that the Friday Sept 18, 2026 CBP deadline has passed. One attribute plus the four text edits the row's own HTML comment prescribes: `data-state="active"` → `"past"`; chip `Effective Sept 18, 2026` → `IN FORCE since Sept 18, 2026`; body `Effective Friday, Sept 18, 2026, CBP will immediately void …` → `As of Sept 18, 2026, CBP voids …`; CTA `Verify your Form 5106 before Friday` → `Verify your Form 5106 now`; and the CTA label drops its now-stale `Pre-deadline` prefix (the same file still says "Verify your Form 5106 now", so the label could not keep promising a deadline that has passed). **Not deleted and not re-typed:** the six required Form 5106 data elements, the five-business-day reinstatement sentence, the notice-*for*-the-action / no-cure-period wording, the four sources and the post href are byte-identical to the pre-flip page. The HTML comment above the row now records that the flip was applied, so a later worker cannot read a pending instruction into an applied one.
- **One date moved, deliberately:** `dateModified` (meta + JSON-LD) `2026-09-15` → `2026-09-19`, together with the row's visible `Updated September 19, 2026`, so the mirror-pair rule the gate enforces stays clean and the page's newest visible "Updated" byline matches the page's newest bytes. The page carries no `datePublished` (unchanged: none before the flip). `sitemap.xml` untouched — its `/` `<lastmod>` has read 2026-09-11 since the head-only `t_a80ddc1a` change, and sitemap reconciliation is owned by `t_290898c5` per the `t_2543361d` ruling.
- **Isolation, measured:** the flip's own diff (`git diff f9cd4aeb^`) is **three hunks** — head dates (lines 24, 35) and the row block (297) — **14 insertions / 10 deletions**, nothing else in the 260 KB document. In a real Chromium DOM the other **6/6** `.notice-banner` rows are byte-identical to their pre-flip `outerHTML`; the **4** `.notice-banner--ior[data-state="past"]` CSS rules are byte-identical to their pre-flip text (the archived styling is the builder's — nothing was restyled); banner count **7 → 7** with the IOR row still first in the stack.
- **Verification (workspace `t_56b4971e/verify_flip.py`, 59/59 assertions):** byte level — no `data-state="active"` anywhere in the file, exactly one element carries any `data-state`, the flipped attribute exactly once, chip / CTA / source-line strings as above, post href present exactly once. Browser level, real Chromium — the row's computed styles equal the archived treatment (box `#f1f5f9`, left rule `#94a3b8`, heading `#475569`, chip `#94a3b8`) and a **flip-back control** sets the attribute to `active` and reproduces the pre-flip computed rendering exactly (amber `#fef2c7` / `#d97706` / `#78350f`) — so the mute is caused by the flip and is reversible; heading and dated chip still **above the fold at 1280×900, 1440×900 and 768×1024** (h2 bottom 564 px, chip bottom 563 px); **zero horizontal overflow** at 320/360/390/414/430/768/1280/1440; the rendered post href is the Sept 18 post and it resolves **200**.
- **Gate + site harnesses:** `site_consistency.py` on the file → **1 checked / 0 errors / 0 warnings**; `tests/ior-checklist-render.py` **53 PASS / 0 FAIL**; `tests/ior-checklist-dom-harness.js` **29 PASS / 0 FAIL** — its pinned `assert.strictEqual(meta, '2026-09-15')` is **replaced by the invariant** (the three date surfaces agree, the value is a valid ISO date, and it can never move older than the page's 2026-09-15 build date), because a pinned date made this harness fail on *every* legitimate page edit; `tariff-data` 127, `copper` 30, `s338` 14/17/28, `trq` 57, `brazil`/`canada-line`/`drone`/`deep-link` ALL CHECKS PASSED.
- **Deploy:** `81be946a-8ffa-460a-a32c-0709e224ea69` (project `tariff-calculator-2026`, branch `main`, commit **`f9cd4ae`**, declared `index.html` only, publisher live verification **59 rows compared / 0 mismatch(es)**). Committed **before** deploying, deliberately: the publisher stages the whole tree from `git HEAD`.
- **Verified live (independent of the publisher):** an `Accept: */*` fetch of `/` is **byte-for-byte identical** to the committed file — **260,066 B, sha256 `54a28a84da9eb056…` on both sides** (`cmp` clean) — the served bytes carry the flipped attribute exactly once and no `active` state; the row's computed rendering, chip text and above-the-fold position were re-measured **in Chromium against the live URL** (identical archived values to the file); and every other live banner row matches the file's. IndexNow **HTTP 200** on `api.indexnow.org` and `bing.com/indexnow` for `/` (key file live **200**).
- **Not done here, on purpose:** the h2's dated label (`— Effective Friday, Sept 18, 2026`) was **not** re-worded — the flip spec names the chip and the two body sentences, and the label remains factually true; the **post page's** own post-deadline tense and `dateModified` belong to **`t_3e502ceb`** (child of this card), which owns `cbp-ior-void-form-5106-september-18.html`; and the calculator's `ior-caption` (line 769, "Updated September 15, 2026") is unchanged — it stamps the calculator's IOR advisory block, whose content did not change.

## 2026-09-18 — Publish /canada-productivity-mega-deduction-tax-rate-2026 (kanban t_669f0fbb) + the two reverse links and the homepage hub entry

- **What shipped:** the publish leg of the Canada Productivity Mega Deduction angle — the writer leg's frozen 40,164 B artifact (`t_28092e1b`) went live at **https://tariffcalculator2026.com/canada-productivity-mega-deduction-tax-rate-2026** (2,081 words prose+FAQ on `verify_post.py`'s instrument; 8 H2; **5 data tables** — 7-row METR/G7 comparator, duty stack, effective tax, year-one write-off, 6-sector Canada-vs-US METRs; **7 FAQ pairs** byte-identical to `faq.json` and mirrored in FAQPage; 18 primary-first sources; 10 briefed internal links), plus **the two brief §5 reverse links** and a **homepage `BLOG_ARTICLES` entry** in a second deploy.
- **Premise carried correctly on the page (the card was wrong in six places, all corrected upstream and preserved here):** the measure is **announced/proposed, not law**; the instrumentation duty on Canadian machinery is **Section 301 at 10% (effective 2026-07-24)** whose USMCA-origin carve-out is the only gate connecting the two sides — **not Section 338** (motor vehicles/alcohol/dairy, 50 per centum ceiling); 6.4% is the **economy-wide METR** on new business investment; 16.9% is **Finance Canada's own estimate** with the Tax Foundation's broader-US-expensing counter-case carried in §7; and there is **no Shopify / Tobi Lütke attribution** (the carded X id 404s against a 200 control in the same call).
- **Deploys:** **`6fda48f8-0f9a-4b39-8408-a6d468fb98c8`** (project `tariff-calculator-2026`, branch `main`, commit **`8c1e882`** — page + sitemap; pre-deploy gate **47 checked / 0 error(s) / 1 warning(s) / 1 pre-existing baselined**; publisher live verification **59 rows compared / 0 mismatch(es)**) and **`9d5482ae-b1ad-4560-bb8e-9537083f1ee0`** (commit **`fdacb7c`** — two sibling pages + `index.html` + `llms.txt`; **59 rows / 0 mismatches**). Both commits were made **before** deploying, path-scoped (`git commit -- <file>`), because the publisher stages the whole tree from `git HEAD` and the index is shared with concurrent workers (a peer's `mybusiness-ai-audit` edits were staged at the time and were left untouched).
- **Publish-day edits, asserted rather than asserted-about:** the PUBLISHER BLOCK's two status chunks were rewritten from "PUBLISH-READY DRAFT" to a truthful `STATUS: PUBLISHED 2026-09-18 by card t_669f0fbb` record (**+537 B**, each old chunk asserted verbatim first); the `<head>` element is **byte-identical** and the `<article>` body text is **squash-identical** to the frozen artifact, so `verify_post.py`'s 103 checks still describe the published bytes (re-run on them: **PASS 103**), together with `site_consistency.py --expect-date 2026-09-18` (**0/0**), `quote_gate.py` (19 spans + negative control clean), `tax_recompute.py` (every dollar figure re-derived in Decimal), `render_check.py` (331 tags, 0 mismatched, h1 1 / h2 8 / h3 4) and `schema_parse_check.py` (**55 rdflib triples**, local parse only — no remote validator pass claimed). Dates needed **no re-stamp**: the frozen stamp day is the ship day.
- **Verified live (independent of the publisher):** plain-`curl` live bytes == the committed file **byte for byte** (40,701 B, sha256 `032a4a70…`), browser-UA fetch +367 B beacon canonicalising to the same file; `/…html` 308s to the extensionless canonical; 1 self-referential canonical, `index, follow`; **FAQPage mainEntity 7 == 7 visible pairs** mirroring character-for-character; **11 distinct internal targets all 200 with distinct payloads**; the brief-forbidden `/section-338-canada-import-ban-usmca-legal-challenge` **not** linked; live sitemap **43 → 44 `<loc>`**; live `llms.txt` carries the URL; real-Chromium layout at **320 / 390 / 1280 px** `scrollWidth == clientWidth` with 0 offending elements; a homepage-card click-through lands on the canonical URL.
- **Publish surfaces:** `sitemap.xml` +1 `<loc>` (`<lastmod>2026-09-18</lastmod>`, weekly, 0.8); `llms.txt` +1 entry (generated by the publisher from the staged tree, label then retargeted to the page title — the generator is additions-only, so the hand label survives); `index.html` `BLOG_ARTICLES` 25 → 26 entries (array re-parsed in node, 0 duplicate files). IndexNow **HTTP 200** for the new page + `/` + the two edited siblings (key file verified live 200 in the same run); the keyword cluster registered for `tariffcalculator2026.com` (**118 → 138** entries, +20 incl. the negative control, backup `keywords.json.bak-20260918-175552`, "no other domain changed" assertion held).
- **Not done here, on purpose:** GSC submission is still impossible from this host (`invalid_grant` — a credentials gap, recorded as such, never as done); the two sibling pages' `dateModified`/byline were **not** re-stamped for a link-only edit; and the staleness triggers (royal assent / a bill number, a Finance revision of 6.4% or 16.9%, a USTR Annex II Part A amendment, a Section 301 Canada rate move, new Section 338 scope reaching machinery) are carded as a chained follow-up rather than left implied.

## 2026-09-15 — IEEPA staleness set: stop pricing a dead authority on 7 pages + homepage authority card (kanban t_b7b7add6)

- **What shipped:** the authority-framing half of the gap report — **11 sentences on 7 pages** that still presented IEEPA as live tariff authority now state the post-2026-02-20 position, plus the **homepage authority card** (gap-report row L1) that gives the hub its primary internal link. Scope was ruled by content-orchestrator (card comment): the **15 named target sentences only**, acceptance (2) amended to *"0 occurrences of IEEPA presented as current authority within those 15 sentences"*, with the wider 90-sentence layer split out as `t_8c8d1145`.
- **Pages touched (7 files, 11 sentences, 1 card):** `/us-tariff-rates-2026-by-country` (U1 intro stack, U2a Layer 3 heading, U2b Layer 3 paragraph, U3a IEEPA-matrix lead-in, U3b matrix forward-look) · `/china-tariff-rates-2026-explained` (U4 lead stack, U5 both IEEPA rows of the five-layer table) · `/tariff-exemption-list-2026-what-products-are-exempt` (U7 — new **Status** column, IEEPA rows *Struck down 2026-02-20*) · `/tariffs-on-electronics-imports-from-asia-2026` (U8 — "three regimes" → two live) · `/tariff-impact-on-us-manufacturing-supply-chains-2026` (U9 — IEEPA bullet marked *no longer available*) · `/how-to-calculate-import-duties-from-china` (U6 — FAQ answer rewritten to *"separate legal authorities, but not all of them are still available"*, visible + FAQPage JSON-LD in parity) · `/index` (U10 — new `.info` card above the drones card). Predicate used throughout: *held unlawful on February 20, 2026 (`Learning Resources, Inc. v. Trump`, No. 24-1287)*, i.e. **IEEPA does not authorise the President to impose tariffs**.
- **Boundaries honoured:** no stacking arithmetic rewritten (the five-layer table's numbers and every other row are byte-identical), no refund mechanics, no Appropriations Clause argument, no per-country rate table. **No new hub anchors were added** — the §7.2 L2-L12 inbound links were landed by `t_993f6b26` and the editor ruled the link half not to be redone; `/index` row L1 was the one row still unowned.
- **Deploy:** `e5e1b8dc-c572-4562-9346-d98e1bd3a4d1` (project `tariff-calculator-2026`, branch `main`, commit **`d17e656`**, 57 staged files, pre-deploy gate **44 checked / 0 error(s) / 1 pre-existing baselined** `404.html` json-ld, publisher live verification **56 rows compared / 0 mismatch(es)**). Committed **before** deploying, deliberately: the publisher stages from `git HEAD`, and `/how-to-calculate-import-duties-from-china` was **live-but-uncommitted** (live `7aa8f3bd…` ≠ HEAD `b4e0719d…`) — a HEAD-staged deploy would have rolled the live page back to the older bytes. That page's previously-live rewrite is carried in this commit, so `HEAD == live` there again.
- **Verified live (acceptance, measured on the served bytes):** (1) **7/7** staleness pages carry a post-2026-02-20 marker *and* the hub link; (2) **15/15** carded sentences free of a live-IEEPA claim; (3) the authority card is in the live homepage bytes, above the drones card, with the planned anchor and the `NOT YET LAW — as of September 15, 2026` marker (homepage hub references **0 → 1**); (4) live `sitemap.xml` and `llms.txt` both carry the hub (added by `t_4715f62d`; `llms.txt` re-read byte-identical to the repo file, 5,348 B); (5) `scripts/build_targets.py` re-run over a **copy** of the `t_92cc79ca` workspace with post-deploy live bytes → **15 targets, 0 missing markers**, and the `update-targets.md` diff shows **9 of the 15 sentences rewritten** (the other 6 were link-only or insertion-above-marker fixes, unchanged by construction); (6) the commit touches **exactly those 7 files** — all same-domain links on them resolve 2xx, hub `200`.
- **Not done here, on purpose:** the **90-sentence layer** — the rate tables and worked examples (the "IEEPA Surcharge" column and Total Effective Rate on `/us-tariff-rates-2026-by-country`, the 16 "IEEPA reciprocal" hits on the exemption page, the `$1,750 on a $1,000 item` arithmetic on the electronics page, the 58.9% component table on `/how-to-calculate-import-duties-from-china`) — is `t_8c8d1145`, which must first classify each row as a live-authority claim vs a description of duty already collected. Also out of scope: `dateModified`/byline re-stamping (a `2026-09-15` ship date is correct in EDT) and the homepage `/news/` surface.
- **Generator caveat (new, measured):** `/how-to-calculate-import-duties-from-china` is **regenerated** by `all-sites-content.py` (its topic is in that site's `topics` list), and that regeneration is what silently dropped `t_993f6b26`'s dated authority qualification from the page on 2026-09-15 22:31 EDT while keeping only its href (carried as a trailing "Related reading" item — the `t_76eafd8f` anchor-inventory contract). A hand-edit to that page is therefore **not durable** against the next run for that topic; the writer-side fix belongs with the `all-sites-content.py` owner, not on this page.

## 2026-09-15 — Publish /presidential-tariff-authority-2026/ (kanban t_4715f62d)

- **What shipped:** the site's new tariff-authority hub at `/presidential-tariff-authority-2026/` — the draft from `t_8941af4b` (32,920 B, `sha256 f758cd9e…`) plus its inbound links from 11 existing pages (`t_993f6b26`), the sitemap entry and the `llms.txt` index line, in **one** deployment.
- **Deploy:** `7e3c7533-07b1-4453-90df-02ecb6bfbfe0` (project `tariff-calculator-2026`, branch `main`, commit **`82d25c0`**, 57 staged files, pre-deploy gate **0 errors / 1 pre-existing baselined**, publisher live verification **56 rows compared / 0 mismatch(es)**).
- **Publish surfaces:** `sitemap.xml` **41 → 42** `<loc>` (hub added with `<lastmod>2026-09-15</lastmod>`/`weekly`/`0.9`; the 11 edited pages' `<lastmod>` bumped to 2026-09-15) and `llms.txt` **41 → 42** entries. The hub line is hand-written **on purpose**: `llms_index.served_pages()` enumerates only root `*.html` for a content site, so a directory-index page is invisible to the generator — same reason `/tariff-refund-status-2026/`, `/tariff-dividend-5000-explained/` and `/news/` are hand lines. The generator is additions-only, so it survives every later publish; it was committed so a HEAD-staged deploy cannot roll it back.
- **Verified live (independent of the publisher):** hub **HTTP 200** on both path forms (non-slash `308` → trailing slash); served bytes **byte-identical** to the committed file (`sha256 f758cd9e…`, raw, on the custom domain and on the project origin); structured data parses clean (`Article` + `FAQPage`, 9 visible Q&A == 9 `mainEntity`, 0 drift, `datePublished` = `dateModified` = 2026-09-15 = the verification date); `robots` `index, follow`; description 140 chars; OG set complete; all **11 internal outbound links 200**; all **11 donor pages serve the hub link**; full sitemap sweep **42/42 locs 200**; no horizontal overflow at 320/360/390/414/430 px; both mechanism tables quotable as plain text. 13 of 14 external links 200 (congress.gov 403s scripted fetches, browser-readable). IndexNow submitted for the hub + 11 pages (`api.indexnow.org` and `bing.com/indexnow` → **HTTP 200**).
- **Status line re-verified before shipping** (the page's headline claim is a dated one): House Clerk roll index still tops out at roll **300**, `rules.house.gov/bill/119/hr-5334` unchanged, Federal Register term counts **0** — so "Not law as of September 15, 2026" stands and the three date fields were not re-stamped. The dated re-check after publication is carded as `t_18d5218c` (content-ops).
- **Not done here, on purpose:** the homepage authority card (gap-report row L1) — `/index` is a concurrent-writer file with three cards pointed at it and it is row L1 of `t_b7b7add6`; the hub therefore has no homepage link yet (measured: 0 hits on the live homepage and on `/news/`). The IEEPA-staleness re-pricing of the 7 pages is also `t_b7b7add6`.

## 2026-09-15 — Homepage description tags trimmed to one intent lead (kanban t_a80ddc1a)

- **What shipped:** the homepage description family is now ONE 143-char intent lead — `meta` **795 → 143**, `og:description` **305 → 143**, `twitter:description` **86 → 143**, byte-identical: *"Free 2026 tariff calculator: Section 301, 232 and 338 rates plus import duties and landed cost by country, product category and shipment value."* Plus the in-head **DESCRIPTION-TAG RULE** comment (the t_406c2d35 house rule) above the tag.
- **Why:** the `meta` tag had grown one clause per tariff action since July — 795 characters whose middle is an accreted event list and whose tail is the calculator's own intent statement, which sat at **char 706 of 795**, i.e. entirely outside the ~160-character SERP window. `og` (305) and `twitter` (86) were two further different strings, so the three description tags disagreed with each other.
- **Not a content deletion.** All 5 tariff events plus the intent were verified item-by-item to have an on-page home *before* the trim, none were removed, and all 13 sampled strings are still on the live page: **Brazil** 25% Section 301 duty (+**1,600** exempt HTSUS subheadings; the post-talks scenario toggle), the **Section 301 forced-labor** 10% / **12.5%** rate on **60 countries**, the **beef** TRQ increase under **Proclamation 11059** (**300,000** metric tons, the "Ground Beef — IN FORCE — Sept 1 to Nov 30, 2026" box), the Canada **Section 338** 50% duty on ~$**20B** of Canadian goods (the "IN EFFECT since Aug 22, 2026" banner) and the Section 232 **drone**/UAS preset (**100%** above **25 kg** or with thermal imaging, 25% otherwise; "took effect **Sept 3**, 2026").
- **Untouched on purpose:** `<meta name="dateModified">` and the JSON-LD `dateModified` both still read **2026-09-15**, as does the visible "Updated September 15, 2026" byline; `sitemap.xml` is unchanged (`/` `<lastmod>` still 2026-09-11) and `llms.txt` was not touched. Head-metadata-only change: `git diff` is 9 added lines (the rule comment) + 3 changed tags.
- **Verification:** house `site_consistency.py` on the file → **1 checked, 0 error(s), 0 warning(s)** (was 2 warnings: `desc-length meta is 795 chars` + the 3-way `desc-parity`); `deploy-idle-site.sh … --dry-run` gate → **43 checked, 0 error(s), 3 warning(s), 1 pre-existing baselined issue(s)** (`DRY RUN: gate passed; nothing deployed.`); deployed **`3335594a-477a-4d9c-a70e-e9fd56e67763`** (commit `c9a6f6f`, **55 live rows compared, 0 mismatch(es)**); independent live re-read of the served bytes: `meta` / `og` / `twitter` = **143 / 143 / 143** and `sha256(live) == sha256(git HEAD) == sha256(worktree)` after `strip_cf_edge_injection` (`b926b1057dd6eaee`).
